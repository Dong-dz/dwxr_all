 ...
 */
import { promisify,request } from './promisify_impl';
...
class LoginController {
  ...
  /**
   * @description: 登录函数，首先调用tt.login获取openid，并向服务端请求登录
  ...
  async login(force = true) {
    ...
    try {
      ...
      //console.log('[login] tt.login success, loginData:', loginData);
    ...
    if (isDouyinLogin && loginData) { //d已经登录抖音和tt.login 
      try {
        // 与后端交互，进行登录
